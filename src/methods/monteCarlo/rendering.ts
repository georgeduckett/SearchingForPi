...
export function drawBackground(ctx: CanvasRenderingContext2D): void {
  ...
  clearCanvas(ctx, s, s)
  drawGrid(ctx, s, s)
  drawCircle(ctx, CIRCLE_RADIUS, CIRCLE_RADIUS, CIRCLE_RADIUS, C_AMBER, 1.5)
}
...
export function drawPoints(ctx: CanvasRenderingContext2D, points: PointResult[]): void {
  ...
  for (const point of points) {
    fillCircle(ctx, point.x, point.y, DOT_RADIUS, point.isInside ? C_INSIDE : C_OUTSIDE)
  }
  ctx.globalAlpha = 1
 ...
 */
export function drawPoint(ctx: CanvasRenderingContext2D, x: number, y: number, isInside: boolean): void {
  ctx.globalAlpha = DOT_ALPHA
