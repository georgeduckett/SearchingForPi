...
// ─── Preview Constants ────────────────────────────────────────────────────────
const PREV_LINE_SPACING = 25      // Spacing between horizontal lines
const PREV_LINE_START = 20        // Y offset for first line
const PREV_NEEDLE_COUNT = 12      // Number of needles to draw
const PREV_NEEDLE_LEN = 20        // Needle length in preview
...
for (let i = 0; i < PREV_NEEDLE_COUNT; i++) {
  // Use deterministic "random" values based on index for stable animation
  ...
  function render(): HTMLElement {
    ...
    page.innerHTML = `
      <header class="page-header">
        <span class="page-index">Method ${getMethodIndex('buffon')}</span>
